import { PrismaClient, UserRole, SessionStatus, BookingStatus, LocationType, ChatType, MessageType, NotificationType, FoodCategory, PaymentStatus, PaymentMethod } from '@prisma/client'
...
async function main() {
  ...
  const venue = await prisma.venue.upsert({
    ...
    create: {
      ...
      description: 'Ambiente aconchegante para suas aventuras de RPG com mesas temáticas e cardápio especial.',
      address: 'Rua dos Aventureiros, 123 - Vila Madalena, São Paulo - SP',
      latitude: -23.5505,
      longitude: -46.6333,
      capacity: 24,
      amenities: ['wifi', 'parking', 'food', 'drinks', 'air_conditioning', 'sound_system'],
  ...
  const sessionPresencial = await prisma.session.upsert({
    ...
    create: {
      ...
      description: 'Uma aventura sombria nas terras de Barovia. Os heróis chegam a uma terra amaldiçoada onde o sol nunca brilha e um vampiro ancestral governa com punho de ferro.',
      gameSystem: 'D&D 5e',
      maxPlayers: 5,
      price: 45.00,
  ...
  const sessionOnline = await prisma.session.upsert({
    where: { id: 'session-cyberpunk-online' },
    update: {},
    create: {
      id: 'session-cyberpunk-online',
      title: 'Cyberpunk RED - Ruas de Night City',
      description: 'Bem-vindos a Night City, onde a tecnologia e a humanidade se chocam. Vocês são edgerunners tentando sobreviver nas ruas perigosas desta metrópole cyberpunk.',
  ...
  const messages = [
    {
      id: 'msg-1',
      content: 'Pessoal, estou muito animado para nossa sessão! Já preparei meu personagem.',
      userId: player.id,
    },
    {
      id: 'msg-2', 
      content: 'Que bom! Lembrem-se de trazer as fichas impressas. Vamos começar pontualmente às 14h.',
      userId: master.id,
    },
    {
      id: 'msg-3',
      content: 'Perfeito! A mesa já está reservada e o cardápio especial estará disponível.',
      userId: storeOwner.id,
    }
  ]
  ...
  await prisma.notification.upsert({
    ...
    create: {
      ...
      message: 'Sua vaga na sessão "A Maldição de Strahd" foi confirmada. Nos vemos no sábado às 14h!',
  ...
  await prisma.notification.upsert({
    ...
    create: {
      ...
      message: 'Recebemos o pagamento de R$ 45,00 referente à sessão "A Maldição de Strahd".',
  ...
  const foodItems = [
    {
      ...
      description: 'Pizza especial com calabresa, queijo e pimenta (para os corajosos!)',
      price: 32.90,
    ...
    },
    {
      id: 'item-poção-mana',
      name: 'Poção de Mana',
      description: 'Refrigerante azul energético para recuperar sua energia mágica',
      price: 8.50,
  ...
  await prisma.review.upsert({
    ...
    create: {
      ...
      sessionId: sessionPresencial.id,
      rating: 5,
      comment: 'Sessão incrível! O Alex é um mestre fantástico e a Caverna do Dragão tem um ambiente perfeito para RPG. Mal posso esperar pela próxima aventura!',
    }
  })
