  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold">Mercado de Aventuras</h1>
          <p className="text-gray-400">Encontre mapas, aventuras prontas e tokens para sua mesa.</p>
        </div>
...
const FilterChip: React.FC<{ label: string; active?: boolean }> = ({ label, active }) => (
  <button className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${active ? 'bg-primary text-white' : 'bg-surface border border-border text-gray-400 hover:border-primary hover:text-white'}`}>
    {label}
  </button>
);
...
const AssetCard: React.FC<{ asset: Asset }> = ({ asset }) => (
  <div className="glass-panel rounded-xl overflow-hidden group cursor-pointer hover:border-primary/50 transition-all">
    <div className="aspect-square bg-cover bg-center relative" style={{backgroundImage: `url('${asset.imageUrl}')`}}>
      <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md px-2 py-1 rounded-md text-xs font-bold flex items-center gap-1">
        <span className="material-symbols-outlined text-[10px] text-accent">star</span> {asset.rating}
      </div>
    ...
    <div className="p-3">
      <div className="flex justify-between items-start mb-1">
        ...
        <span className="text-sm font-bold text-green-400">R$ {asset.price.toFixed(2)}</span>
      </div>
      <h3 className="font-bold text-white leading-tight mb-2 line-clamp-2">{asset.title}</h3>
