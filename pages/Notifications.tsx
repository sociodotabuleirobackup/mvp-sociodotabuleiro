    ...
    <div className="max-w-3xl mx-auto p-4 animate-fade-in">
      <div className="flex justify-between items-end mb-6">
        ...
        <div className="flex gap-4 items-center">
            ...
            <button className="text-xs text-primary hover:underline">Marcar todas como lidas</button>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {notifications.length === 0 ? (
          ...
          notifications.map(notif => (
            ...
            >
              <div className={`w-10 h-10 rounded-full bg-white/5 flex items-center justify-center shrink-0 ${getColor(notif.type)}`}>
                <span className="material-symbols-outlined">{getIcon(notif.type)}</span>
              </div>
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <h3 className={`text-sm font-bold mb-1 ${notif.read ? 'text-gray-400' : 'text-white'}`}>
                    {notif.title}
                  </h3>
                  <span className="text-[10px] text-gray-600">{notif.date}</span>
                </div>
