  ...
  return (
    <div className="min-h-screen bg-background p-6 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10 animate-fade-in">
        ...
        <h2 className="text-3xl font-display font-bold mb-2">Escolha seu Destino</h2>
        <p className="text-gray-400 mb-8">Como você deseja participar da comunidade?</p>
    ...
    <div className="bg-background min-h-screen text-white overflow-x-hidden selection:bg-primary selection:text-white">
      <nav className="absolute top-0 w-full p-6 flex justify-between items-center z-50">
        <div className="flex items-center gap-2">
           ...
           <span className="font-fantasy font-bold text-lg tracking-wider hidden sm:block text-white">Sócio do Tabuleiro</span>
        </div>
        <button 
          onClick={onLogin} 
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">
              A BUROCRACIA É NOSSA. <span className="text-primary">A MAGIA É SUA.</span>
            </h2>
          </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-fantasy font-bold text-xl text-white">Sócio do Tabuleiro</h3>
      ...
      <ul className="space-y-3">
        {features.map((feat, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-gray-400">
            <span className={`material-symbols-outlined text-base ${highlight ? 'text-accent' : 'text-primary'}`}>check</span>
            {feat}
          </li>
        ))}
      </ul>
...
const TechItem: React.FC<{ icon: string; title: string; desc: string }> = ({ icon, title, desc }) => (
...
const Testimonial: React.FC<{ quote: string; author: string; role: string }> = ({ quote, author, role }) => (
  <div className="p-6 rounded-2xl bg-surface border border-white/5 relative">
    <span className="material-symbols-outlined absolute top-4 left-4 text-4xl text-white/5 font-serif">format_quote</span>
    <p className="text-gray-300 italic mb-6 relative z-10">"{quote}"</p>
    <div>
      <p className="font-bold text-white font-display">{author}</p>
      <p className="text-xs text-primary-light uppercase tracking-wide">{role}</p>
    </div>
  </div>
);
