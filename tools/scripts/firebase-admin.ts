...
if (getApps().length > 0) {
  ...
  if (process.env.FIREBASE_SERVICE_ACCOUNT_JSON) {
    try {
      ...
      console.error(
        '❌ Erro ao fazer parse do FIREBASE_SERVICE_ACCOUNT_JSON:',
        e
      );
    }
  }
  // 2. Tenta carregar via Caminho de Arquivo (Bom para Dev Local)
    ...
    if (fs.existsSync(keyPath)) {
      try {
        ...
        credential = cert(serviceAccount);
        console.log(
          `🔒 Firebase Admin: Autenticado via arquivo (${process.env.FIREBASE_SERVICE_ACCOUNT_PATH}).`
        );
      } catch (e) {
  ...
  if (!credential) {
    console.log(
      '⚠️ Firebase Admin: Usando Application Default Credentials ou Emulador.'
    );
  }
