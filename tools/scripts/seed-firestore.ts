...
if (!isEmulator && !allowProd) {
  console.warn(
    '⚠️  ATENÇÃO: Você está prestes a rodar o seed no projeto REAL:',
    projectId
  );
...
async function main() {
  console.log(
    `🌱 Seeding Firestore | Project: ${projectId} | Emulator: ${isEmulator}`
  );
  ...
  batch.set(venueRef, {
    ...
    description:
      'O melhor lugar para jogar RPG em SP. Temos lanches e ar condicionado.',
  ...
  batch.set(sessionRef, {
    ...
    system: 'D&D 5e',
    description:
      'Uma aventura gótica nas terras de Barovia. Traga sua coragem (e fichas nível 3).',
